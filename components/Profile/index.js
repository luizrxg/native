...
export default function Componente(){
  ...
  const profilePosts = posts.filter(e => e.user === selectedProfile?.user).reverse()
  const following = []
  follows?.map((val) => {
  ...
  return (
    <View style={[styles.profile, {backgroundColor: colors?.primary}]}>
      ...
      {selectedProfile !== loggedProfile &&
      <View style={styles.buttonsContainer}>
        {followers?.includes(loggedProfile?.user) ?
        <Button
          ...
          onPress={() => {
            setFollows(
              follows?.filter(e =>
                JSON.stringify(e) !==
                JSON.stringify({
                  follower: loggedProfile?.user,
                  followed: selectedProfile?.user
                })
              )
            )
          }}
        ...
        <Button
          color={colors?.secondary}
          style={[styles.button, {borderColor: colors?.accent, backgroundColor: colors?.accent}]}
          onPress={() => setFollows(arr => [...arr, {follower: loggedProfile?.user, followed: selectedProfile?.user}])}
        >
          {following?.includes(loggedProfile?.user) ? 'Seguir de volta' : 'Seguir'}
        </Button>}
        <Button
          color={colors?.secondary}
          style={[styles.button, {borderColor: colors?.secondary}]}
          onPress={() => {
            setSelectedChat(selectedProfile?.user)
            navigate('Chat')
        }}>
          Mensagem
        </Button>
